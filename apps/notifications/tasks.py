...
def send_single_notification_task(self, raw):
    ...
    group(
        ...
        send_push_notification_task.s(
            p.user_id, p.title, p.message, p.notification_type, p.extra_data
        ),
    ).apply_async()
...
def send_push_notification_task(
    self, user_id, title, message, notification_type="system", extra_data=None
):
    if not firebase_admin._apps:
        logger.info(
            "FCM: Firebase ilovasi initsializatsiya qilinmagan, push yuborilmadi."
        )
    ...
    for i in range(0, len(tokens), FCM_BATCH_SIZE):
        ...
        try:
            ...
            invalid_tokens += [
                batch[j] for j, r in enumerate(response.responses) if not r.success
            ]
        except Exception as e:
    ...
    if invalid_tokens:
        ...
        logger.warning(
            "FCM: %d ta eskirgan token o'chirildi | user=%s", deleted, user_id
        )
