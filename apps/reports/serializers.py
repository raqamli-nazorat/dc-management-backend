...
class UserComprehensiveReportSerializer(serializers.ModelSerializer):
    ...
    def setup_eager_loading(queryset):
        ...
        def get_meeting_subquery(attended=None, excused=None):
            meeting_filter = Q(meeting__project__isnull=True) | Q(
        ...
        def get_payroll_subquery(field):
            ...
            return Subquery(
                Payroll.objects.filter(user=OuterRef("pk"), is_active=True)
                .order_by()
                .values("user")
...
class ProjectComprehensiveReportSerializer(serializers.ModelSerializer):
    ...
    def setup_eager_loading(queryset):
        ...
        return (
            queryset.select_related("created_by", "manager")
...
class ExpenseRequestReportSerializer(serializers.ModelSerializer):
    ...
    expense_category = serializers.CharField(
        source="expense_category.title", read_only=True
    )
    ...
    def setup_eager_loading(queryset):
        return queryset.select_related(
            "user", "accountant", "project", "expense_category"
        )
