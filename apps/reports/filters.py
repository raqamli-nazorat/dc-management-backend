...
class UserReportFilter(filters.FilterSet):
    ...
    position = NumberInFilter(
        field_name="position_id", lookup_expr="in", label="Lavozim"
    )
    roles = filters.CharFilter(method="filter_roles", label="Rollar")

    joined_min = filters.DateTimeFilter(
        field_name="date_joined", lookup_expr="gte", label="Qo'shilgan vaqt (dan)"
    )
    joined_max = filters.DateTimeFilter(
        field_name="date_joined", lookup_expr="lte", label="Qo'shilgan vaqt (gacha)"
    )

    balance_min = filters.NumberFilter(
        field_name="balance", lookup_expr="gte", label="Balans (min)"
    )
    balance_max = filters.NumberFilter(
        field_name="balance", lookup_expr="lte", label="Balans (max)"
    )

    salary_min = filters.NumberFilter(
        field_name="fixed_salary", lookup_expr="gte", label="Maosh (min)"
    )
    salary_max = filters.NumberFilter(
        field_name="fixed_salary", lookup_expr="lte", label="Maosh (max)"
    )

    projects_min = filters.NumberFilter(
        method="filter_projects_count", label="Loyihalar soni (min)"
    )
    projects_max = filters.NumberFilter(
        method="filter_projects_count", label="Loyihalar soni (max)"
    )
    project_status = filters.ChoiceFilter(
        choices=ProjectStatus.choices, method="filter_dummy", label="Loyiha holati"
    )

    tasks_min = filters.NumberFilter(
        method="filter_tasks_count", label="Vazifalar soni (min)"
    )
    tasks_max = filters.NumberFilter(
        method="filter_tasks_count", label="Vazifalar soni (max)"
    )
    task_status = filters.ChoiceFilter(
        choices=TaskStatus.choices, method="filter_dummy", label="Vazifa holati"
    )

    meetings_min = filters.NumberFilter(
        method="filter_meetings_count", label="Yig'ilishlar soni (min)"
    )
    meetings_max = filters.NumberFilter(
        method="filter_meetings_count", label="Yig'ilishlar soni (max)"
    )
    ...
    expenses_amount_min = filters.NumberFilter(
        method="filter_expenses_amount", label="Xarajatlar (min)"
    )
    expenses_amount_max = filters.NumberFilter(
        method="filter_expenses_amount", label="Xarajatlar (max)"
    )
    ...
    payrolls_amount_min = filters.NumberFilter(
        method="filter_payrolls_amount", label="Ish haqi (min)"
    )
    payrolls_amount_max = filters.NumberFilter(
        method="filter_payrolls_amount", label="Ish haqi (max)"
    )
    ...
    def filter_projects_count(self, queryset, name, value):
        ...
        queryset = queryset.annotate(
            total_projects=(
                Count(
                    "created_projects", filter=get_q("created_projects"), distinct=True
                )
                + Count(
                    "manager_projects", filter=get_q("manager_projects"), distinct=True
                )
                + Count(
                    "employee_projects",
                    filter=get_q("employee_projects"),
                    distinct=True,
                )
                + Count(
                    "tester_projects", filter=get_q("tester_projects"), distinct=True
                )
            )
        )
    ...
    def filter_tasks_count(self, queryset, name, value):
        ...
        queryset = queryset.annotate(
            total_tasks=Coalesce(
                Subquery(subquery, output_field=IntegerField()), Value(0)
            )
        )
    ...
    def filter_meetings_count(self, queryset, name, value):
        ...
        subquery = (
            MeetingAttendance.objects.filter(filters_q)
            .values("user")
            .annotate(cnt=Count("pk"))
            .values("cnt")
        )
    ...
    def filter_expenses_amount(self, queryset, name, value):
        ...
        subquery = (
            ExpenseRequest.objects.filter(filters_q)
            .values("user")
    ...
    def filter_payrolls_amount(self, queryset, name, value):
        ...
        subquery = (
            Payroll.objects.filter(
                user=OuterRef("pk"), is_active=True, is_confirmed=True
            )
            .values("user")
...
class ProjectReportFilter(filters.FilterSet):
    ...
    deadline_min = filters.DateTimeFilter(
        field_name="deadline", lookup_expr="gte", label="Muddati (dan)"
    )
    deadline_max = filters.DateTimeFilter(
        field_name="deadline", lookup_expr="lte", label="Muddati (gacha)"
    )

    price_min = filters.NumberFilter(
        field_name="project_price", lookup_expr="gte", label="Boshqaruvchi bonusi (dan)"
    )
    price_max = filters.NumberFilter(
        field_name="project_price",
        lookup_expr="lte",
        label="Boshqaruvchi bonusi (gacha)",
    )

    created_by = NumberInFilter(
        field_name="created_by_id", lookup_expr="in", label="Mualliflar"
    )
    manager = NumberInFilter(
        field_name="manager_id", lookup_expr="in", label="Boshqaruvchilar"
    )
    employees = NumberInFilter(
        field_name="employees", lookup_expr="in", label="Xodimlar"
    )
    testers = NumberInFilter(
        field_name="testers", lookup_expr="in", label="Sinovchilar"
    )
...
class ExpenseReportFilter(filters.FilterSet):
    ...
    accountant = NumberInFilter(
        field_name="accountant_id", lookup_expr="in", label="Hisobchi"
    )
    project = NumberInFilter(field_name="project_id", lookup_expr="in", label="Loyiha")
    expense_category = NumberInFilter(
        field_name="expense_category_id", lookup_expr="in", label="Kategoriya"
    )

    type = CharInFilter(field_name="type", lookup_expr="in", label="Xarajat turi")
    payment_method = CharInFilter(
        field_name="payment_method", lookup_expr="in", label="To'lov turi"
    )
    status = CharInFilter(field_name="status", lookup_expr="in", label="Holati")

    amount_min = filters.NumberFilter(
        field_name="amount", lookup_expr="gte", label="Miqdori (min)"
    )
    amount_max = filters.NumberFilter(
        field_name="amount", lookup_expr="lte", label="Miqdori (max)"
    )
    ...
    paid_at_min = filters.DateTimeFilter(
        field_name="paid_at", lookup_expr="gte", label="To'langan vaqt (dan)"
    )
    paid_at_max = filters.DateTimeFilter(
        field_name="paid_at", lookup_expr="lte", label="To'langan vaqt (gacha)"
    )

    confirmed_at_min = filters.DateTimeFilter(
        field_name="confirmed_at", lookup_expr="gte", label="Tasdiqlangan vaqt (dan)"
    )
    confirmed_at_max = filters.DateTimeFilter(
        field_name="confirmed_at", lookup_expr="lte", label="Tasdiqlangan vaqt (gacha)"
    )

    cancelled_at_min = filters.DateTimeFilter(
        field_name="cancelled_at", lookup_expr="gte", label="Bekor qilingan vaqt (dan)"
    )
    cancelled_at_max = filters.DateTimeFilter(
...
class PayrollReportFilter(filters.FilterSet):
    ...
    is_confirmed = filters.BooleanFilter(
        field_name="is_confirmed", label="Tasdiqlanganmi?"
    )

    month_year = filters.CharFilter(
        method="filter_by_month_year", label="Oy va yil (YYYY-MM)"
    )

    total_amount_min = filters.NumberFilter(
        field_name="total_amount", lookup_expr="gte", label="Jami miqdori (min)"
    )
    total_amount_max = filters.NumberFilter(
        field_name="total_amount", lookup_expr="lte", label="Jami miqdori (max)"
    )

    salary_min = filters.NumberFilter(
        field_name="fixed_salary", lookup_expr="gte", label="Oylik maosh (min)"
    )
    salary_max = filters.NumberFilter(
        field_name="fixed_salary", lookup_expr="lte", label="Oylik maosh (max)"
    )

    penalty_min = filters.NumberFilter(
        field_name="penalty_amount", lookup_expr="gte", label="Jarima miqdori (min)"
    )
    penalty_max = filters.NumberFilter(
        field_name="penalty_amount", lookup_expr="lte", label="Jarima miqdori (max)"
    )

    kpi_min = filters.NumberFilter(
        field_name="kpi_bonus", lookup_expr="gte", label="KPI bonus (min)"
    )
    kpi_max = filters.NumberFilter(
        field_name="kpi_bonus", lookup_expr="lte", label="KPI bonus (max)"
    )

    created_at_min = filters.DateTimeFilter(
        field_name="created_at", lookup_expr="gte", label="Hisoblangan vaqt (dan)"
    )
    created_at_max = filters.DateTimeFilter(
        field_name="created_at", lookup_expr="lte", label="Hisoblangan vaqt (gacha)"
    )
...
class TaskReportFilter(filters.FilterSet):
    ...
    assignee = NumberInFilter(
        field_name="assignee_id", lookup_expr="in", label="Topshiruvchi"
    )
    created_by = NumberInFilter(
        field_name="created_by_id", lookup_expr="in", label="Yaratuvchi"
    )
    position = NumberInFilter(
        field_name="position_id", lookup_expr="in", label="Lavozim"
    )
    ...
    penalty_min = filters.NumberFilter(
        field_name="penalty_percentage", lookup_expr="gte", label="Jarima foizi (min)"
    )
    penalty_max = filters.NumberFilter(
        field_name="penalty_percentage", lookup_expr="lte", label="Jarima foizi (max)"
    )

    price_min = filters.NumberFilter(
        field_name="task_price", lookup_expr="gte", label="Vazifa narxi (min)"
    )
    price_max = filters.NumberFilter(
        field_name="task_price", lookup_expr="lte", label="Vazifa narxi (max)"
    )
    ...
    reopened_min = filters.NumberFilter(
        field_name="reopened_count", lookup_expr="gte", label="Qaytishlar soni (min)"
    )
    reopened_max = filters.NumberFilter(
        field_name="reopened_count", lookup_expr="lte", label="Qaytishlar soni (max)"
    )
