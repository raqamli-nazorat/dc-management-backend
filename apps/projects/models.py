...
class Project(BaseModel):
    ...
    is_deleted = models.BooleanField(
        default=False, db_index=True, verbose_name="O'chirilganmi?"
    )
    is_hidden = models.BooleanField(
        default=False, db_index=True, verbose_name="Yashirilganmi?"
    )
    ...
    manager = models.ForeignKey(
        ...
        limit_choices_to={"roles__contains": [Role.MANAGER]},
        db_index=True,
    ...
    def clean(self):
        ...
        if self.pk:
            ...
            if (
                old_project.status in [ProjectStatus.COMPLETED, ProjectStatus.CANCELLED]
                and not is_soft_delete_action
            ):
    ...
    def save(self, *args, **kwargs):
        ...
        if not is_new:
            if self.is_hidden and not self._old_is_hidden:
                ...
                if (
                    self.status == ProjectStatus.OVERDUE
            ...
            if (
                self.deadline != self._old_deadline
                and self.is_hidden == self._old_is_hidden
            ):
                if (
                    self.deadline > timezone.now()
                    and self.status == ProjectStatus.OVERDUE
                ):
                    self.status = ProjectStatus.ACTIVE
...
class Task(BaseModel):
    uid = models.CharField(
        max_length=20, unique=True, editable=False, verbose_name="UID"
    )
    project = models.ForeignKey(
        Project, on_delete=models.PROTECT, related_name="tasks", verbose_name="Loyiha"
    )
    title = models.CharField(max_length=255, verbose_name="Nomi")
    description = models.TextField(null=True, blank=True, verbose_name="Tavsifi")

    rejection_reason = models.TextField(
        null=True, blank=True, verbose_name="Rad etish sababi"
    )
    ...
    task_price = models.DecimalField(
        max_digits=12, decimal_places=2, default=0.00, verbose_name="Vazifa narxi"
    )
    penalty_percentage = models.DecimalField(
        max_digits=5,
        decimal_places=2,
    ...
    sprint = models.PositiveSmallIntegerField(
        null=True,
        blank=True,
    ...
    def _is_current_user_admin(self):
        ...
        return bool(
            ...
            and (user.is_superuser or user.has_role(Role.ADMIN))
        )
    ...
    def clean(self):
        ...
        if self.pk:
            try:
                old_task = Task.objects.only("project_id", "status", "assignee_id").get(
                    pk=self.pk
                )
            ...
            if old_task.project_id != self.project_id:
                raise ValidationError(
                    {"project": "Vazifa boshqa loyihaga ko'chirilishi mumkin emas!"}
                )
            ...
            if (
                not is_admin
                and old_task.status in locked_statuses
                and old_task.assignee_id != self.assignee_id
            ):
                raise ValidationError(
                    {
        ...
        if self.assignee and self.project_id:
            if not self.project.employees.filter(id=self.assignee_id).exists():
                raise ValidationError(
                    {"assignee": "Xodim loyiha a'zolari ro'yxatida topilmadi!"}
                )
...
class TaskAttachment(BaseModel):
    ...
    file = models.FileField(
        upload_to="tasks/files/", validators=[validate_file_size], verbose_name="Fayl"
    )
...
class Meeting(BaseModel):
    ...
    recording_url = models.URLField(
        null=True, blank=True, verbose_name="Yozuv havolasi"
    )
    ...
    is_completed = models.BooleanField(
        default=False, db_index=True, verbose_name="Tugatildimi?"
    )
    completed_at = models.DateTimeField(
        null=True, blank=True, verbose_name="Tugallangan vaqt"
    )
    is_deleted = models.BooleanField(
        default=False, db_index=True, verbose_name="O'chirilganmi?"
    )
    ...
    notification_eta = models.DateTimeField(
        null=True, blank=True, verbose_name="Rejalashtirilgan eta"
    )
...
class MeetingAttendance(BaseModel):
    ...
    is_attended = models.BooleanField(
        default=False, db_index=True, verbose_name="Qatnashdimi?"
    )
    is_excused = models.BooleanField(
        default=False, db_index=True, verbose_name="Sabablimi?"
    )
    payroll_processed = models.BooleanField(
    ...
    absence_reason = models.TextField(
        null=True, blank=True, validators=[MinLengthValidator(10)], verbose_name="Sabab"
    )
