...
def handle_project_post_save(sender, instance, created, **kwargs):
    ...
    if instance.is_hidden and not _old_is_hidden:
        ...
        not instance.is_hidden
        and _old_is_hidden
        and getattr(instance, "_was_unfrozen", False)
    ):
        ...
        if working_seconds > 0:
            ...
            transaction.on_commit(
                lambda: update_project_tasks_on_unlock.delay(
                    instance.id, working_seconds
                )
            )
...
def handle_project_employees_change(sender, instance, action, pk_set, **kwargs):
    if action in ["pre_add", "pre_remove"]:
        is_superuser = (
            instance._is_current_user_superuser()
            if hasattr(instance, "_is_current_user_superuser")
            else False
        )
        if instance.status == ProjectStatus.CANCELLED or (
            instance.status == ProjectStatus.COMPLETED and not is_superuser
        ):
