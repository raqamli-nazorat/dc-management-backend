...
class ExpenseRequestViewSet(
    ...
    def pay_expense(self, request, pk=None):
        ...
        return Response(
            {
                "message": "To'lov muvaffaqiyatli amalga oshirildi. Xodimning tasdiqlanishi kutilmoqda."
            },
            status=status.HTTP_200_OK,
        )
    ...
    def confirm_receipt(self, request, pk=None):
        ...
        return Response(
            {"message": "Xarajatlar muvaffaqiyatli tasdiqlandi."},
            status=status.HTTP_200_OK,
        )


@extend_schema(tags=["Expense Receipts"])
class ExpenseReceiptViewSet(
    SoftDeleteMixin, RoleBasedQuerySetMixin, viewsets.ModelViewSet
):
    ...
    def perform_create(self, serializer):
        ...
        if expense.status != Status.PAID:
            raise ValidationError(
                {"status": "Chekni faqat to'langan so'rovlarga yuklash mumkin."}
            )
    ...
    def perform_destroy(self, instance):
        ...
        if instance.expense.status != Status.PAID:
            raise PermissionDenied(
                "Faqat to'langan so'rov cheklarini o'chirish mumkin."
            )
...
class PayrollViewSet(RoleBasedQuerySetMixin, viewsets.ReadOnlyModelViewSet):
    ...
    def confirm_payroll(self, request):
        ...
        confirmed_count = PayrollService.confirm_payrolls(
            payroll_ids=payroll_ids, accountant_user=request.user
        )
