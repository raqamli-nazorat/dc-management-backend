...
class ExpenseRequestSerializer(ModelCleanMixin, serializers.ModelSerializer):
    ...
    expense_category_info = ExpenseCategorySerializer(
    ...
    def __init__(self, *args, **kwargs):
        ...
        if request and hasattr(request, "user") and request.user.is_authenticated:
            ...
            if hasattr(user, "roles") and not (
                user.is_superuser or user.has_role(*privileged_roles)
            ):
    ...
    def validate(self, attrs):
        ...
        for field in ["project", "card_number", "expense_category"]:
            if field in attrs or getattr(instance, field) is None:
...
class PayrollSerializer(ModelCleanMixin, serializers.ModelSerializer):
