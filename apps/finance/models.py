...
class ExpenseRequest(BaseModel):
    ...
    type = models.CharField(
        ...
        default=ExpenseType.WITHDRAWAL,
        db_index=True,
        verbose_name="Turi",
    )
    ...
    cancel_reason = models.TextField(
        null=True, blank=True, verbose_name="Bekor qilish sababi"
    )
    ...
    paid_at = models.DateTimeField(
        null=True, blank=True, verbose_name="To'langan vaqti"
    )
    confirmed_at = models.DateTimeField(
        null=True, blank=True, verbose_name="Tasdiqlangan vaqti"
    )
    cancelled_at = models.DateTimeField(
        null=True, blank=True, verbose_name="Bekor qilingan vaqti"
    )
    ...
    def clean(self):
        ...
        if not self.pk:
            has_pending = ExpenseRequest.objects.filter(
                user=self.user, status__in=[Status.PENDING, Status.PAID], is_active=True
            ).exists()
...
class Ledger(BaseModel):
    ...
    amount = models.DecimalField(
        max_digits=12, decimal_places=2, verbose_name="Miqdori"
    )
...
class Payroll(BaseModel):
    ...
    fixed_salary = models.DecimalField(
        max_digits=12, decimal_places=2, verbose_name="Oylik maosh"
    )
    kpi_bonus = models.DecimalField(
        max_digits=12, decimal_places=2, default=0, verbose_name="KPI bonusi"
    )
    penalty_amount = models.DecimalField(
        max_digits=12, decimal_places=2, default=0, verbose_name="Jarima miqdori"
    )
    total_amount = models.DecimalField(
        max_digits=12, decimal_places=2, editable=False, verbose_name="Jami miqdori"
    )

    tasks_completed = models.PositiveIntegerField(
        default=0, verbose_name="Bajarilgan vazifalar"
    )
    deadline_missed = models.PositiveIntegerField(
    ...
    missed_meetings_count = models.PositiveIntegerField(
    ...
    is_confirmed = models.BooleanField(
        default=False, db_index=True, verbose_name="Tasdiqlandimi?"
    )
    confirmed_at = models.DateTimeField(
        null=True, blank=True, verbose_name="Tasdiqlangan vaqti"
    )
    ...
    def clean(self):
        ...
        if self.pk:
            ...
            if old_instance.is_confirmed:
                if (
                    old_instance.fixed_salary != self.fixed_salary
                    or old_instance.kpi_bonus != self.kpi_bonus
                    or old_instance.penalty_amount != self.penalty_amount
                ):
                    raise ValidationError(
