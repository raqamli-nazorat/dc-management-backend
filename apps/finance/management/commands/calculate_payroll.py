    ...
    def handle(self, *args, **options):
        ...
        if not month_raw:
            try:
                ...
                month_input = input(
                    "Hisoblanadigan oyni kiriting (masalan: 2026-08, o'tgan oyni olish uchun bo'sh qoldiring): "
                ).strip()
                if month_input:
        ...
        self.stdout.write(
            self.style.MIGRATE_HEADING(
                f"  OYLIK MAOSHLARNI HISOBLASH: {display_name} ({month_str})"
            )
        )
        if user_id:
            self.stdout.write(
                self.style.NOTICE(
                    f"  Tanlangan foydalanuvchi: {user_obj.username} (ID: {user_obj.id})"
                )
            )
        self.stdout.write(self.style.MIGRATE_HEADING("=" * 70))
        ...
        for detail in stats.get("details", []):
            ...
            if st == "confirmed":
                ...
                self.stdout.write(
                    self.style.ERROR(f" [X] {u.username:<20} | Xatolik: {err}")
                )
            else:
                payroll = detail["payroll"]
        ...
        self.stdout.write(
            f"  - Jami hisoblangan foydalanuvchilar: {stats['processed']}"
        )
        self.stdout.write(
            self.style.SUCCESS(
                f"  - Yangi yaratilgan:                  {stats['created']}"
            )
        )
        self.stdout.write(
            self.style.SUCCESS(
        ...
        if stats["skipped_confirmed"] > 0:
            self.stdout.write(
                self.style.NOTICE(
                    f"  - Tasdiqlangan (o'tkazilgan):        {stats['skipped_confirmed']}"
                )
            )
        if stats["errors"] > 0:
            self.stdout.write(
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"  - Jami hisoblangan summa:            {stats['total_amount']:,.2f} so'm"
            )
        )
        self.stdout.write(self.style.MIGRATE_HEADING("=" * 70))
        ...
        if stats["errors"] == 0:
            ...
            self.stdout.write(
                self.style.WARNING(
                    "Hisob-kitob yakunlandi, lekin ayrim xatoliklar yuz berdi."
                )
            )
